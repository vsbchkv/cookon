...
import { selectData, setData } from '../../../features/data-slice/data-slice';

const Calculator: React.FC = () => {
  const [currentId = '', setCurrentId] = useState<string | undefined>();
  const [name, setName] = useState('');
  const [category, setCategory]: [Item['category'], unknown] = useState('ingredients');
  ...
  const handleCreateItem = useCallback(async () => {
    ...
    await createItem({ item: obj, endpoint: obj.category });
    const data = useSelector(selectData);
    setData({ ...data, [category]: [...data[category], obj] });
  }, [createItem, name, category, currentId]);
  // console.log(useSelector(selectData));
